...
def build_C_star_all(theta_vec, C, w, LM, M, alpha=0.4):
    """
    构造所有 h 的候选 C_star
    """
    gon_threshold=0.95
...
def fun_gon(theta_vec, C, w, LM, M, alpha, gon_threshold):
    """约束：Gon >= gon_threshold"""
    try:
        C_star = build_C_star_all(theta_vec, C, w, LM, M, alpha)
...
def outer_objective_max(theta_vec, C_current, C_original, w, LM, M, tau, nu, alpha=0.4):
    """
    外层目标函数（最大化），C_current 会迭代更新
    C_original 永远是初始的 C（目标函数参考基准）
    """
    try:
        C_star = build_C_star_all(theta_vec, C_current, w, LM, M, alpha)
    except Exception:
        return 1e10
...
def outer_constraints(C, w, LM, M, alpha=0.4, gon_threshold=0.95):
    """生成约束列表"""
    return [{
        'type': 'ineq',
        'fun': lambda theta: fun_gon(theta, C, w, LM, M, alpha, gon_threshold)
    }]


def optimize_layer(C, w, LM, M, tau, nu, alpha=0.4, gon_threshold=0.95):
    """
    外层优化主函数
    ...
    constraints = outer_constraints(C_current, w, LM, M, alpha, gon_threshold)

    def callback(theta):
        """每次迭代更新 C_current"""
        nonlocal C_current
        C_current = build_C_star_all(theta, C_current, w, LM, M, alpha)

    res = minimize(
        fun=outer_objective_max,
        x0=x0,
        args=(C_current, C_original, w, LM, M, tau, nu, alpha),
        method='SLSQP',
        bounds=bounds,
...
if __name__ == "__main__":
    ...
    result = optimize_layer(C, w, LM, M, tau, nu, alpha=alpha, gon_threshold=gon_threshold)
    ...
    if hasattr(result, 'x') and result.x is not None:
        ...
        print("使用的 θ (N×3):\n", theta_final)
        try:
            C_star_final = build_C_star_all(result.x, C, w, LM, M, alpha=alpha)
    ...
    for h in range(C.shape[0]):
        if Con_base_final[h] < gon_threshold:
            AD = AD + 1
    print("AD：", AD)
